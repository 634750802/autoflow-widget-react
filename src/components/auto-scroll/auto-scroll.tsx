...
import { AutoScrollContext, type ScrollEdge, type ScrollHandler } from './context';
...
export function AutoScroll ({ target, edgePixels = 0, children }: AutoScrollProps) {
  ...
  const [handlers, setHandlers] = useState<RefObject<ScrollHandler>[]>([]);
  ...
  useEffect(() => {
    ...
    if (scrollTarget) {
      const measureTarget = scrollTarget instanceof Window ? document.documentElement : scrollTarget;
